...
def add_routes_bus():
    ...
    db.engine.execute(BusRoute.__table__.insert(), val)
    try:
        db.engine.execute('CREATE INDEX ix_bus_routes_route_id_index ON bus_route (route_id)')
        db.session.commit()
...
def add_stops_bus():
    ...
    db.engine.execute(BusStop.__table__.insert(), val)
    try:
        db.engine.execute("CREATE INDEX ix_bus_stops_stop_id_index ON bus_stop (stop_id)")
        db.session.commit()
    except Exception as e:
...
def add_trips_bus():
    ...
    db.engine.execute(BusTrip.__table__.insert(), val)
    try:
        db.engine.execute("CREATE INDEX ix_bus_trips_trip_id_index ON bus_trip (trip_id)")
        db.session.commit()
    except Exception as e:
...
def add_stop_times_bus():
    ...
    db.engine.execute(BusStopTime.__table__.insert(), val)
    try:
        db.engine.execute("CREATE INDEX ix_bus_stop_times_trip_id_index ON bus_stop_time (trip_id)")
        db.session.commit()
    except Exception as e:
        print(f'add_stop_times_bus {e}')
    print('Added Stop Time.')
...
def generate_routes_bus():
    ...
    for r in bus_routes_df.route_id:
        ...
        db.session.commit()
    try:
        db.engine.execute("CREATE INDEX ix_bus_all_routes_route_id_index ON bus_all_route (route_id)")
    except Exception as e:
...
def set_route_details_bus():
    ...
    for r in bus_routes_df.route_id:
        try:
            ...
            db.session.add(route_details)
            db.session.commit()
        except Exception as e:
            print(f'set_route_details_bus {e}')
    try:
        db.engine.execute("CREATE INDEX ix_bus_routes_details_route_id_index ON bus_routes_detail (route_id)")
    except Exception as e:
        print(f'set_route_details_bus {e}')
...
def set_transit_route_stops_dist_bus():
    ...
    for r in bus_routes_df.route_id:
        ...
        # print(r)
    try:
        db.engine.execute("CREATE INDEX ix_bus_route_stops_distance_route_id_index ON bus_route_stop_distance"
                          "(route_id)")
    except Exception as e:
        print(f'add_routes_bus {e}')
...
def set_next_stop_bus():
    ...
    for s in all_stops:
        ...
        db.session.add(next_stop)
        db.session.commit()

    try:
        db.engine.execute("CREATE INDEX ix_bus_next_stop_cur_stop_index ON bus_next_stop (cur_stop)")
    except Exception as e:
        print(f'add_routes_bus {e}')
